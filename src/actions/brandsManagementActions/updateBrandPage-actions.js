import auth from "../../auth";
import { baseURI } from "../../config";
import * as layoutActions from "../layout-actions";
import { fetchSubscriptions } from "../subscriptionsManagementActions/subscriptionsPage-actions";
import { fetchBrandDetails } from "./brandDetailsPage-actions";
import { normalizeFetchedData } from "../../helperFunctions";
import { validURL } from "../../helperFunctions";
import { baseURIImage } from "../../config";
export const fetchInitialData = (brandID) => {
  return async (dispatch, getState) => {
    try {
      dispatch(isLoading(true));
      const [brandDetails, subscriptions] = await Promise.all([
        fetchBrandDetails(brandID),
        fetchSubscriptions({
          search: "",
          nextTo: 0,
          order: true,
          numOfResults: 6400,
        }),
      ]);
      dispatch({
        type: "updateBrandPage-subscriptions",
        data: subscriptions,
      });
      dispatch(placeFormData(normalizeFetchedData(brandDetails)));
      dispatch(isLoading(false));
    } catch (error) {
      dispatch(
        layoutActions.handleHttpError(error, {
          reload: true,
          willGoBack: true,
        })
      );
      dispatch(isLoading(false));
    }
  };
};
const normalizeSubscriptionBrands = (subscriptionBrands) => {
  return [].concat(
    ...subscriptionBrands.map((item, key) => {
      return {
        SubscriptionId: item.Subscription.Id,
        Rank: item.Rank,
        IsAvailable: item.IsAvailable,
      };
    })
  );
};
const placeFormData = (brandDetails) => {
  return (dispatch, getState) => {
    const brandImageFile = getState().updateBrandPage_reducer.formData
      .brandImageFile.value;
    dispatch(
      changeFormFieldValue(
        {
          ...brandImageFile,
          data: `${baseURIImage}/img/brands/100x100/${brandDetails.ImagePath}`,
        },
        "brandImageFile"
      )
    );
    dispatch(changeFormFieldValue(brandDetails.Name, "name"));
    dispatch(changeFormFieldValue(brandDetails.NameEn, "nameEn"));
    dispatch(
      changeFormFieldValue(brandDetails.ShortDescription, "shortDescription")
    );
    dispatch(
      changeFormFieldValue(brandDetails.LongDescription, "longDescription")
    );
    dispatch(
      changeFormFieldValue(
        brandDetails.ShortDescriptionEn,
        "shortDescriptionEn"
      )
    );
    dispatch(
      changeFormFieldValue(brandDetails.LongDescriptionEn, "longDescriptionEn")
    );
    dispatch(changeFormFieldValue(brandDetails.IsAvailable, "isAvailable"));
    dispatch(changeFormFieldValue(brandDetails.URL, "URL"));
    dispatch({
      type: "updateBrandPage-subscriptionBrands",
      data: normalizeSubscriptionBrands(brandDetails.SubscriptionBrands),
    });
  };
};
export const uploadBrandImage = (brandImageFile) => {
  return async (dispatch) => {
    try {
      dispatch({ type: "updateBrandPage-isUploadingBrandImage", data: true });
      const imageId = await sendUploadBrandImage(brandImageFile.value);
      dispatch(
        changeFormFieldValue({ ...brandImageFile, imageId }, "brandImageFile")
      );
      dispatch({
        type: "updateBrandPage-isUploadingBrandImage",
        data: false,
      });
    } catch (error) {
      dispatch({
        type: "updateBrandPage-isUploadingBrandImage",
        data: false,
      });
      dispatch(layoutActions.handleHttpError(error));
    }
  };
};
export const changeFormFieldValue = (value, id) => {
  return (dispatch, getState) => {
    dispatch({
      type: `updateBrandPage-${id}-value`,
      data: { value, id },
    });
    if (getState().updateBrandPage_reducer.formData[id].errorMsg !== "") {
      dispatch({
        type: `updateBrandPage-${id}-errorMsg`,
        data: { value: "", id },
      });
    }
  };
};

export const isLoading = (isLoading) => {
  return (dispatch) => {
    dispatch({ type: "updateBrandPage-isLoading", data: isLoading });
  };
};

const sendUploadBrandImage = (imageFile) => {
  return new Promise(async (resolve, reject) => {
    var myHeaders = new Headers();
    myHeaders.append("Authorization", `Bearer ${auth.userData.access_token}`);
    var formdata = new FormData();
    formdata.append("file", imageFile);

    var requestOptions = {
      method: "POST",
      headers: myHeaders,
      body: formdata,
      redirect: "follow",
    };
    try {
      const response = await fetch(
        `${baseURI}/api/Brand/Image/Upload`,
        requestOptions
      );
      const body = JSON.parse(await response.text());
      if (response.status === 200) {
        resolve(body.ImageId);
      } else {
        reject({ code: body?.errorCode, message: body?.message });
      }
    } catch (error) {
      reject(error);
    }
  });
};

export const submitForm = (brandID) => {
  return async (dispatch, getState) => {
    try {
      const formData = getState().updateBrandPage_reducer.formData;
      const subscriptionBrands = getState().updateBrandPage_reducer
        .subscriptionBrands;
      dispatch(isLoading(true));
      const imageId = formData.brandImageFile.value.imageId;
      await sendFormData(brandID, formData, subscriptionBrands, imageId);
      dispatch(clearFormFields());
      dispatch(
        layoutActions.alertModal({
          show: true,
          willGoBack: true,
        })
      );
      dispatch(isLoading(false));
    } catch (error) {
      // dispatch(postSubmitValidation(error ? error.errorCode : null))
      dispatch(layoutActions.handleHttpError(error));
      dispatch(isLoading(false));
    }
  };
};
export const sendBrandState = (brandID, isAvailable) => {
  return (dispatch) => {
    return new Promise(async (resolve, reject) => {
      var myHeaders = new Headers();
      myHeaders.append("Authorization", `Bearer ${auth.userData.access_token}`);
      myHeaders.append("Content-Type", "application/x-www-form-urlencoded");
      var urlencoded = new URLSearchParams();
      urlencoded.append("IsAvailable", isAvailable);
      urlencoded.append("Id", brandID);
      var requestOptions = {
        method: "PUT",
        headers: myHeaders,
        redirect: "follow",
        body: urlencoded,
      };
      try {
        const response = await fetch(
          `${baseURI}/api/Brand/UpdateAvailability`,
          requestOptions
        );
        const body = JSON.parse(await response.text());
        if (response.status === 200) {
          resolve(body);
        } else {
          reject({ code: body?.errorCode, message: body?.message });
        }
      } catch (error) {
        reject(error);
      }
    });
  };
};

export const updateBrandState = (brandID, isAvailable) => {
  return async (dispatch) => {
    try {
      dispatch(isLoading(true));
      await dispatch(sendBrandState(brandID, isAvailable));
      dispatch(changeFormFieldValue(isAvailable, "isAvailable"));
      dispatch(
        layoutActions.alertModal({
          show: true,
          body: !isAvailable
            ? "تم إخفاء العلامة التجارية عن المستخدمين بنجاح"
            : "تم إظهار العلامة التجارية للمستخدمين بنجاح",
        })
      );
      dispatch(isLoading(false));
    } catch (error) {
      dispatch(layoutActions.handleHttpError(error));
      dispatch(isLoading(false));
    }
  };
};

const sendFormData = (brandID, formData, subscriptionBrands, imageID) => {
  return new Promise(async (resolve, reject) => {
    var myHeaders = new Headers();
    myHeaders.append("Authorization", `Bearer ${auth.userData.access_token}`);
    myHeaders.append("Content-Type", "application/json");
    const body = JSON.stringify({
      Id: brandID,
      Name: formData.name.value, // length >= 2 && <= 49
      NameEn: formData.nameEn.value, //length >= 2 && <= 49
      ShortDescription: formData.shortDescription.value, // length  <= 249
      LongDescription: formData.longDescription.value, // length  <= 1500
      ShortDescriptionEn: formData.shortDescriptionEn.value, // length  <= 249
      LongDescriptionEn: formData.longDescriptionEn.value, // length  <= 1500
      URL: formData.URL.value ?? new URL(formData.URL.value), // Valid URL => new URL()
      ImageId: imageID ? imageID : -1, // Optinal, if user uploaded an image, place the Id here
      SubscriptionBrands: subscriptionBrands,
    });
    var requestOptions = {
      method: "PUT",
      headers: myHeaders,
      redirect: "follow",
      body,
    };

    try {
      const response = await fetch(
        `${baseURI}/api/Brand/Update`,
        requestOptions
      );
      const body = JSON.parse(await response.text());
      if (response.status === 200) {
        resolve(body);
      } else {
        reject({ code: body?.errorCode, message: body?.message });
      }
    } catch (error) {
      reject(error);
    }
  });
};
export const clearBrandImageFile = () => {
  return (dispatch) => {
    dispatch(
      changeFormFieldValue(
        { value: "", name: "", data: "", imageId: "", path: "" },
        "brandImageFile"
      )
    );
  };
};
const clearFormFields = () => {
  return async (dispatch, getState) => {
    const formData = getState().updateBrandPage_reducer.formData;
    for (let key in formData) {
      let value;
      if (key === "brandImageFile") {
        dispatch(clearBrandImageFile());
        continue;
      } else if (
        key === "isAvailable" ||
        key === "isSubscriptionBrandAvailable"
      ) {
        value = true;
      } else {
        value = "";
      }
      dispatch({
        type: `createBrandPage-${key}-value`,
        data: { value, id: key },
      });
      dispatch({ type: "updateBrandPage-subscriptionBrands", data: [] });
    }
  };
};

export const preSubmitValidation = (cb) => {
  return (dispatch, getState) => {
    const formData = getState().updateBrandPage_reducer.formData;
    let isFormValid = true;
    if (formData.URL.value !== "" && !validURL(formData.URL.value)) {
      isFormValid = false;
      dispatch({
        type: `updateBrandPage-URL-errorMsg`,
        data: {
          value: "عنوان الموقع الإلكتروني غير صحيح",
          id: "URL",
        },
      });
    }
    if (formData.name.value !== "" && formData.name.value.length < 4) {
      isFormValid = false;
      dispatch({
        type: `updateBrandPage-name-errorMsg`,
        data: {
          value: "الإسم لايجب أن يكون أقل من 4 أحرف",
          id: "name",
        },
      });
    }

    if (formData.nameEn.value !== "" && formData.nameEn.value.length < 4) {
      isFormValid = false;
      dispatch({
        type: `updateBrandPage-nameEn-errorMsg`,
        data: {
          value: "الإسم لايجب أن يكون أقل من 4 أحرف",
          id: "nameEn",
        },
      });
    }

    for (let key in formData) {
      if (formData[key].config.required && formData[key].value === "") {
        isFormValid = false;
        dispatch({
          type: `updateBrandPage-${key}-errorMsg`,
          data: { value: "يجب تعبئة هذا الحقل", id: key },
        });
      }
    }

    cb(isFormValid);
  };
};

const postSubmitValidation = (errorCode) => {
  return (dispatch) => {
    switch (errorCode) {
      default:
        dispatch(
          layoutActions.alertModal({
            isSuccess: false,
            show: true,
          })
        );
    }
  };
};
