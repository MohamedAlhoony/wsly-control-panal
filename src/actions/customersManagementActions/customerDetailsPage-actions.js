...
export const fetchInitialData = (customerID) => {
  return async (dispatch, getState) => {
    try {
      ...
      dispatch({
        type: "customerDetailsPage-customerDetails",
        data: normalizeFetchedData(customerDetails),
      });
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      var response = await fetch(
        `${baseURI}/api/Customer/${customerID}`,
        requestOptions
      );
...
export const isLoading = (isLoading) => {
  return (dispatch) => {
    dispatch({ type: "customerDetailsPage-isLoading", data: isLoading });
  };
};
